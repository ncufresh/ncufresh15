...
<div id="ckeditor">
    ...
    <select id="mySelect" class="browser-default">
        ...
        <option value="" disabled selected>重要日程</option>
    	...
    	<option value="{{ $cate_17->id }}">{{ $cate_17->title }}</option>
    	@endforeach

    	<option value="" disabled selected>文件及下載專區</option>
    	@foreach($cate_18 as $cate_18)
    	<option value="{{ $cate_18->id }}">{{ $cate_18->title }}</option>
    	@endforeach

        <option value="" disabled selected>聯絡我們</option>
        @foreach($cate_19 as $cate_19)
        <option value="{{ $cate_19->id }}">{{ $cate_19->title }}</option>
        @endforeach
    </select>
    <br>
